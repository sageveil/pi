...
import { enableFuzzyFiles } from './fuzzy-files.ts';
import { VimEditor, type VimMode } from './vim.ts';
...
const readConfig = async (): Promise<{ config: SageveilConfig; warning?: string }> => {
  ...
  return {
    config: {
      vim: boolean(values.vim, defaults.vim),
      fuzzyFiles: boolean(values.fuzzyFiles, defaults.fuzzyFiles),
      statusline: {
        icon: boolean(statusline.icon, statuslineDefaults.icon),
        vimMode: boolean(statusline.vimMode, statuslineDefaults.vimMode),
  ...
  pi.on('session_start', async (_event, ctx) => {
    ...
    if (config.vim) {
      if (ctx.ui.getEditorComponent()) {
        ctx.ui.notify('sageveil: Vim disabled; another editor is active', 'warning');
      } else {
        vimMode = 'insert';
    ...
    if (config.fuzzyFiles) await enableFuzzyFiles(pi, ctx);
    ...
    ctx.ui.setFooter((tui, theme, footerData) => ({
      ...
      render(width: number): string[] {
        ...
        const left = [
          ...
          config.statusline.vimMode && vimMode &&
            theme.fg(vimMode === 'insert' ? 'success' : 'warning', vimMode.toUpperCase()),
